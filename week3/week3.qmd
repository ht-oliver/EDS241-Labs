...
weekly_la_phx <- did_data_az_ca %>% 
  mutate(
    date_obj = mdy(date), 
    # Identify the week, round down to monday
    week = floor_date(date_obj, unit = "week"),
  ) %>% 
  # Filter to february, march and april of 2020
  filter(year == 2020, month(date_obj) %in% c(2,3,4)) %>% 
  # One data entry per week per city
  group_by(week, city) %>% 
  # Calculate mean of NO2
...
**No we cannot blame the difference we see on the lockdown, because these lines were not parallel before the lockdown. THere is a huge spike in Late february in LA that is not mimicked by Phoenix**
...
**Our trends are similar, so this violates our parallel assumption situation much less. We may proceed.**
...
y_trt_post <- did_data_nm_tx %>% filter(treated == 1, is_post == 1, year == 2020, month(date_obj) %in% c(2, 3, 4)) %>% 
  summarize(mean(daily_max_1_hour_no2_concentration)) %>% pull()

y_cnt_post <- did_data_nm_tx %>% filter(treated == 0, is_post == 1, year == 2020, month(date_obj) %in% c(2, 3, 4)) %>% 
  summarize(mean(daily_max_1_hour_no2_concentration)) %>% pull()

y_trt_pre <- did_data_nm_tx %>% filter(treated == 1, is_post == 0, year == 2020, month(date_obj) %in% c(2, 3, 4)) %>% 
  summarize(mean(daily_max_1_hour_no2_concentration)) %>% pull()

y_cnt_pre <- did_data_nm_tx %>% filter(treated == 0, is_post == 0, year == 2020, month(date_obj) %in% c(2, 3, 4)) %>% 
  summarize(mean(daily_max_1_hour_no2_concentration)) %>% pull()


trt_effect <- ((y_trt_post - y_trt_pre) - (y_cnt_post - y_cnt_pre))
...
New Mexicans (more strict lockdown) receive 3.5162 less NO2 per hour ppb per month than less strict lock-downers (Texans)
...
**I like the lack of violation in my parallel assumption, and the geographic similarities between the regions. These results could be do to random chance or other political/economic factors. Also, our data is over a short time-span, and our data is pretty volatile, so this could effect the accuracy of our model.**
...
**treated:ispost, the p value indicates this relationship could be replicated by random chance 22% of the time.**
...
**El Paso is our counterfactual. Dust storm could ruin effect, different cities could have different dust storm infrastructure. But if the effect of the dust-storm is the same on each city, we could still observe our effect.**
