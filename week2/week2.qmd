...
Intercept: Without any treatment, we expect the number of treat piles to be ~78 on average.
...
summary(m3_log, model.fil = FALSE)
```
Intercept: On average, there are approximately 75 waste piles in control group / untreated neighborhoods. (exp(intercept))

Coefficient: On average, there is a 34% reduction in waste piles in the treated neighborhoods.
  (exp(-0.42 - 1)) * 100
...
Intercept: exp(0.47) = 1.6. When both treatment and the pre-count average are at 0, there are 1.6 waste piles.

Coefficient: 24% increase in trash piles when treatment occurs, and we control for pre-count average at 1
...
export_summs(m3_log, m4_control,
             model.names = c("Log OLS Regression", "Log OLS Regression with Control"),
                             statistics= "none")
