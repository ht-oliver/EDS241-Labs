...
rdd_lm <- ggplot(data_binned, aes(x = avg_distance, y = avg_deforest, color = as.factor(protected))) +
  ...
  geom_smooth(method = "lm", aes(gorup = protected)) +
...
**RESPONSE**:
The red line indicates that rates of deforestation decrease as you approach the border of protected areas, have a sharp decline once you enter protected areas, and then continue to decrease at a lower rate as you push further into protected areas. The discontinuity indicates that the protected areas have a distinct effect on deforestation. If there was no discontinuity, we might assume that protected areas do not have an effect on deforestation, rather decreases are just from increasing distance into forests.
...
**RESPONSE**:
Protected status decreases rates of deforestation by 10%. This is consistent with the figure above. The interaction coefficient represents the effect that distance has on the effect that protection has on deforestation. In this case, the slope in protected areas is slightly higher than the slope in non-protected areas.
...
global_rdd <- rdrobust(
  ...
  covs = global_samp %>% select(slope_cat, road_cat, water_cat, soil_cat), # Select covariates
  c = 0,
  kernel = "triangular"
)
...
**RESPONSE**:
We specified a cutoff with our kernel, which means we're looking at far fewer observations, but these observations are ideal for identifying the effect of our treatment. We are increasing our error - but zooming in closer on the part of the effect that we care about. This model accounts for weighting, optimizes the bandwidth, and allows the relationship to be non-linear. We should trust it more than our OLS - because OLS assumes that our variance in error is consistent across our sample - this model accounts for variability in our variance of error.
...
**RESPONSE**:
Poly plot is not linear, so it can capture our relationship with higher specificity
...
**RESPONSE**:
Similar effects as sean in the Neal plot - DRC does not find a dramatic effect from our treatment, the most extreme effects are seen in Malaysia, Brazil and Canada. Canada's rate of deforestation in protected areas are effected heavily for some other factor - Neal suspects it's wildfire. Our graph shows this effect, so we can assume something similar. There are heavier dives near the border in the Neal studies, our data shows less dramatic dives in deforestation approaching the protected area border.
