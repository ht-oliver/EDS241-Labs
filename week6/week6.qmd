...
- Factors that could effect both PM2.5 and Inhaler Searches
- If there's a widespread sickness more people could be hoarding inhalers, and traveling less
- Wildfire - lots of smoke.
...
```{r}
options(noaakey = "NdlbSenGFEaBUjWcaTClaAqtMxKeNJyl")
```
...
ggplot(data_weekly_joined, aes(x = precip_total, y = pm25_mean)) +
  geom_point(alpha = 0.6, color = "darkgreen", size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "dodgerblue", linewidth = 1) +
  labs(x = "Weekly Total Precip (Inches)",
       y = "Asthma PM2.5",
       title = "Precipitation vs PM 2.5 (First Stage Relationship)") +
  theme_minimal()
...
**RESPONSE: The coefficient is about 2.5 larger in the FE example. This means that seasonality was confounding the relationship between pm2.5 and asthma searches - but there are still other confounding factors. The p-value got closer to 1 with our fixed effects, indicating reduced statistical power.  Bias probably exists in weekly traffic patterns (school/work) and wildfire events.**
...
**RESPONSE: Yes, negative - precipitation takes particulate out of the air. This is a weak instrument. Fstat is below 10. Fstat represents unexplained variance.**
