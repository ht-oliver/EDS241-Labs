...
**RESPONSE:** with multiple states you can pick and choose factors that represent California. By limiting ourselves to one state we guarantee that we'll be making sacrifices in some of our observed and unobserved covariates. Apples to oranges between California and Texas, but if we use multiple states we can choose just the apples from each state
...
**RESPONSE:** We want the synthetic control to closely track the pre-treatment period, if it doesn't track then the synthetic state is probably not a good match the real state. This would make our results useless
...
**RESPONSE:** This ensures that we're getting more accurate measurements of our outcome post-treatment. We don't want to be estimating co2 before AND after our treatment - we want to have baseline, more accurate measurements for co2 pre-treatment. Since we have them - we should use them.
...
scm_data <- data %>%
  synthetic_control(
    outcome = co2_power_pc_tons, # Outcome variablel: per capita CO2
    unit = state, # Unit identifier (state)
    time = year, # Time identifier (year)
    i_unit = "CA", # intervention (treated) unit: CA
    i_time = 2013, # Intervention (treatment) year: 2013
    
    
    # Generate pacebo tests for all states
    generate_placebos = TRUE) %>%
  
  
  
  # Defining characteristics synthetic California must match in order to be considered a good copy of real California
  
  generate_predictor(  
    time_window = 2000:2012, # pre-treatment period only to compute predictors
  ...
  # Take the predictor summaries from above and actually figure out how much weight to give each donor state
  ...
  # Construct synthetic CA using the optimize donor weights
  generate_control() 
```
...
scm_data %>% 
  plot_trends() +
  labs(title = "California Duuuuuude hhahaha syntheticc brah",
       y = "CO2 per Capita",
       x = "Year")
...
**RESPONSE:** Yes they do - they fit the trend but not that annual changes. This means we can use it to predict longer-term trends, our synthetic control is useful.

2.  What happens to the gap between the two lines after 2013? What does this suggest about the effect of cap-and-trade?

**RESPONSE:** The lines split, this means that the policy was effective in reducing emissions.
...
**RESPONSE:** Pre-treatment has is generally declining with medium oscillation on a yearly basis. After treatment there is a steep decline in CO2, and oscillation because more dramatic and has higher variation. Oscillations take longer after treatment.
...
**RESPONSE:** It's number 1 by a factor of 5! This means that the simulated data is working well for most states, and that our treatment had significant effects on our outcome variable.
...
**RESPONSE:** Super well! The elec_price is the worst but still comparable.

2.  How does the synthetic control compare to the simple donor pool average?

**RESPONSE:** All numbers are on the same magnitude. Synthetic california is closer to CA than to the donor_sample, which implies our weights are functioning
...
**RESPONSE:** The cap and trade program was functional. We made a statistically robust synthetic california, and proved that it was capable of serving as a counterfactual. Then our figures illustrated efficacy of the treatment.
...
**RESPONSE:** Strong fit - check, transparent donor pool - not great, no spillover - could be spillover, favorable placebo comparison - check.
...
**RESPONSE:** We would have seen similar effects in other states and they would have clouded the result in California. If we're interpreting states that have received a treatment like they're untreated, it will make our analysis of treatment in California less accurate.
...
**RESPONSE:** We included pre-treatment outcome.
