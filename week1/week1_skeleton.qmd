...
balance_summary <- class_data %>% 
  gtsummary::tbl_summary(
    ...
    statistic = list(all_continuous() ~ "{mean} ({sd})") # What's in the quotes is formatting for results
  ) %>% 
  # Add p value
  add_p()
...
balance_summary %>% 
  as_kable_extra(caption = "Clas Survey Balance Table") %>% 
...
```{r}
tbl_2way %>% # Use gt() if you're coming from a dataframe. Use as_gt() if you're coming from gt summary table
  gt(rowname_col = "year") %>% 
  tab_header(
...
tbl_3way <- lobsters %>% 
  ...
  summarize(total_cpue = sum(cpue, na.rm = TRUE),
            .groups = "drop") %>%  # Ungroup
  # Pivot to create a column for each treat/control group within each region
...
tbl_3way %>% # Use gt() if you're coming from a dataframe. Use as_gt() if you're coming from gt summary table
  gt(rowname_col = "year") %>% 
  tab_header(
  ...
  cols_label( # label labels individual columns
    bol_con = "Control",
    bol_mpa = "MPA",
    flo_con = "Control",
    flo_mpa = "MPA",
  ...
  tab_spanner( # Spanner labels multiple columns
    label = "Flødevigen",
...
table_w_plots <- lobsters %>% 
  ...
  summarize(
    total_cpue = sum(cpue, na.rm = TRUE),
    dist_cpue = list(cpue), # store individual cpeu values as a list for each year
    .groups = "drop"
  ) %>% 
  arrange(year) %>% 
  # Create gt table
